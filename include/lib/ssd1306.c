...
void SSD1306_Resize (uint8_t resize){
	_size = resize;
}
...
uint8_t SSD1306_DrawChar (char character)
{
  ...
  if (_size == 1){
	...
	for (int selectChar = 0; selectChar < CHARS_COLS_LENGTH*2; selectChar++)
	{
		
		uint16_t resizeChar = 0;

		uint8_t fontByte = pgm_read_byte(&FONTS[character-32][selectChar/2]);
			
		unsigned int mask = 1U;
		
		for (int k = 0; k < 8; k++) {
			charArr[k] = (fontByte & mask) ? 1U : 0U;
			mask = mask << 1U;
		}
		
		for (uint8_t loadBit = 0; loadBit < 16; loadBit += 1){
			resizeChar |= charArr[(loadBit/2)] << loadBit;
		}
		
		if (_counter%128 >= 120) return 10;
		
		cacheMemLcd[_counter] = (resizeChar & 0x00FF);
		cacheMemLcd[_counter + 128] = (resizeChar & 0xFF00) >> 8;
...
void SSD1306_DrawString (char str[])
{
  ...
  while (str[i]) {
    // draw string
	if (SSD1306_DrawChar (str[i++])) break;
  }
